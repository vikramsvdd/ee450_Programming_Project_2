...
int main() {
    ...
    if (!file.is_open()) {
        cerr << "Error opening file: dataC.txt" << endl;
        return 1;
    }
  ...
  bzero((char*)&mainServerAddr, sizeof(mainServerAddr));
    ...
    serverAddr.sin_port = htons(32074);  
bind(udpSocket, (struct sockaddr*)&serverAddr, sizeof(serverAddr));
	cout<<" Server C is up and running using UDP on port 32074"<<endl;
    ...
    while (true) {
        ...
        recvfrom(udpSocket, buffer, sizeof(buffer), 0, (struct sockaddr*)&mainServerAddr, &mainServerAddrLen);
         cout<<"Server C has received a request for ";
		cout<<buffer<<endl;
...
string deptid="Student IDs: ";     // string to store the dept ids
            ...
            string response = "Server C found  " + to_string(uniqueStudentCount) + " distinct student(s) for "+departmentName+" "+deptid;
			...
			cout<<"Server C has sent the result to Main Server"<<endl;
			}
